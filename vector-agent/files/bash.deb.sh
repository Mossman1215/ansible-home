...
function config_url {
    echo "https://repositories.timber.io/public/vector/config.deb.txt?distro=${distro}&codename=${codename}&version=${version}&arch=${arch}" | sed 's/ /%20/g'
}
...
function import_gpg_key {
    local text="Importing 'timber/vector' repository GPG key into apt ..."
    echo_running "$text"
    curl -1sLf "https://repositories.timber.io/public/vector/gpg.3543DB2D0A2BC4B8.key" | apt-key add - &>$tmp_log
    echo_okfail "$text" || die "Could not import the GPG key for this repository"
}

function setup_repository {
    local repo_path="/etc/apt/sources.list.d/timber-vector.list"
    check_fetch_config

    local text="Installing 'timber/vector' repository via apt ..."
    echo_running "$text"
